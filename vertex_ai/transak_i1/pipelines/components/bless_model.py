...
def bless_model_op(
    candidate_model: Input[VertexModel],
    project: str,
    location: str,
):
    """
    ...
    new_production_model = aiplatform.Model(model_name=candidate_model.resource_name)
    new_production_model.set_version_aliases(aliases=['production'])
    print(f"Model {new_production_model.resource_name} blessed with 'production' alias.")

    # # Get the model resource name from the input artifact
    # with open(candidate_model.path, 'r') as f:
